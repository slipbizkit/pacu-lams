...
export default function SupportStaffHistoryPage() {
  ...
  async function handleSearch() {
    ...
    setCurrentPage(1);
    try {
      const data = await clientService.listAllHistory({ search: search || undefined });
      setRows(data);
    } catch (err) {
              ...
              <tbody>
                {pageRows.map((row) => (
                  <tr key={row.client_id}>
                    ...
                    <td><span className="pacu-mono fw-semibold">{row.queue_number}</span></td>
                    <td>
                    ...
                    <td className="text-muted">{row.employer || <span style={{ opacity: 0.4 }}>—</span>}</td>
                    <td className="text-muted">{row.lawyer_name || <span style={{ opacity: 0.4 }}>—</span>}</td>
                    <td className="text-muted" style={{ maxWidth: 220 }}>
                      <div className="text-truncate">{row.issue_categories || <span style={{ opacity: 0.4 }}>—</span>}</div>
                    </td>
                  </tr>
                ))}
              </tbody>
